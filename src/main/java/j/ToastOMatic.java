...
class Butterer implements Runnable{
    ...
    public Butterer(ToastQueue dryToasts, ToastQueue butteredToast, Sandwich s){
        sandwich=s;
...
class Jeller implements Runnable{
    ...
    public Jeller(ToastQueue dryToasts, ToastQueue jelledToast, Sandwich s){
        sandwich=s;
        this.dryToasts=dryToasts;
...
class ToastButterJelly extends Toast{
    ...
    private Toast jelly;
    public ToastButterJelly(Toast butter, Toast jelly, Sandwich s){
        super(butter.getId());
        sandwich=s;
        this.butter=butter;
        this.jelly=jelly;
    ...
    public void run(){
        try {
            while (!Thread.interrupted()){
                ...
                ToastButterJelly toast = new ToastButterJelly(t, t2, this);
                synchronized (this) {
                    while (!toast.isButtered() || !toast.isJelled()) {
                        wait();
                    }
                    toast.sandwich();
                    System.out.println("sandwich with peanut butter and jelly " + t2.getId());
...
public class ToastOMatic {
    public static void main(String[] args) throws InterruptedException {
        ...
        exec.execute(s=new Sandwich(butteredToast, jelledToast, finishedQueue));
        exec.execute(new Butterer(dryQueue,butteredToast,s));
        exec.execute(new Jeller(dryQueue, jelledToast,s));
